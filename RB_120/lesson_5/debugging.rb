...
Not necessary. Because there is no differences between them. Super takes the code from Animal
and there is no other lines, that means they are the same.
...
class Person
  ...
  # def same_location?(other_person)
  #   location.latitude == other_person.location.latitude &&
  #   location.longitude == other_person.location.longitude
  # end
end
...
# puts ada.same_location?(grace) # expected: true
                                 # actual: false
...
class BankAccount
  ...
  def withdraw(amount)
    if (amount > 0) && valid_transaction?(balance - amount)#(amount <= self.balance)
...
example = Example.new.example = 'lucy'
puts example
...
class TaskManager
  ...
  def display_high_priority_tasks
    new_tasks = tasks.select do |task|
      task.priority == :high
    end
